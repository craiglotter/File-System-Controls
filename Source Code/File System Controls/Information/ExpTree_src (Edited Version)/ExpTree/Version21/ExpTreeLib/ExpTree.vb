...
#Region "       SelectItem"
    ...
    Public Function SelectItem(ByVal baseItem As CShItem, ByVal targetitem As CShItem) As Boolean
        SelectItem = False     'assume failure
        Dim baseNode As TreeNode = Root
        tv1.BeginUpdate()
        baseNode.Expand() 'Ensure base is filled in
        'do the drill down -- Node to expand must be included in tree
        Dim testNode As TreeNode
        Dim lim As Integer = CShItem.PidlCount(baseItem.PIDL) - CShItem.PidlCount(baseNode.Tag.pidl)
        'TODO: Test ExpandARow again on XP to ensure that the CP problem ix fixed
        Do While lim > 0
            For Each testNode In baseNode.Nodes
                If CShItem.IsAncestorOf(testNode.Tag, baseItem, False) Then
                    baseNode = testNode
                    RefreshNode(baseNode)   'ensure up-to-date
                    baseNode.Expand()
                    lim -= 1
...
NEXLEV: Loop
        ...
        For Each testNode In baseNode.Nodes
            If testNode.Tag.ToString() = targetitem.ToString Then
                Me.tv1.SelectedNode = testNode
                SelectItem = True
                Exit For
            End If
        Next
...
#Region "   RightClick"
    Private Sub tv1_MouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles tv1.MouseDown
        If e.Button = MouseButtons.Right Then
            Dim node As TreeNode = tv1.GetNodeAt(e.X, e.Y)
            If Not node Is Nothing Then
                Me.tv1.SelectedNode = node
            End If
        End If
    End Sub
#End Region
